...
import { FaSun, FaCloud, FaTint, FaSnowflake, FaThermometerHalf, FaWind, FaEye, FaTachometerAlt, FaCloudSun, FaMoon, } from "react-icons/fa";
...
export default function CurrentLocWeatherDataCard(): ReactElement {
  ...
  if (!context || !context.weatherAppData?.apiStates?.data) {
    return <div><LoadingSpinner /></div>;
  }

  const { weatherAppData } = context;

  const data = weatherAppData.apiStates.data;
  const currentCondition = data?.current_condition[0];
  const nearestArea = data?.nearest_area[0];
  const todayWeather = data?.weather[0];

  const weatherDesc = currentCondition.weatherDesc[0].value;
  const backgroundImage = getWeatherImage("sunny");
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="relative rounded-2xl overflow-hidden h-full">
        {
          weatherAppData?.apiStates?.loading === false ?
            <div>
              <img
                src={backgroundImage}
